...
const CACHE_FILE = path.join(CACHE_DIR, "blogs.json");
...
const initialBlogsCache = await readCacheFile();

if (Array.isArray(initialBlogsCache)) {
  blogsCache = initialBlogsCache;
}
...
async function fetchPublicBlogsFromSupabase() {
...
async function refreshBlogsCache() {
  const freshData = await fetchPublicBlogsFromSupabase();
...
router.get("/", async (_req, res) => {
  try {
    const data = await loadCachedBlogs();

    console.log("Blogs served:", data.length);
...
router.get("/:id", async (req, res) => {
  try {
    const cachedData = await loadCachedBlogs();
    ...
    const reloadedData = await loadCachedBlogs();
...
router.post("/", async (req, res) => {
  try {
    ...
    return res.status(201).json(normalizeBlog(data));
  } catch (error) {
...
router.put("/:id", async (req, res) => {
  try {
    ...
    return res.json(normalizeBlog(data));
  } catch (error) {
